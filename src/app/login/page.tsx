  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (userRole === 'ngo') {
        ...
        window.location.href = '/'; // Donors and others go to the main page
      }
    } catch (err: any) {
      ...
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full p-3 border border-gray-300 text-gray-900 rounded-lg" />
          </div>
          <div className="mb-6">
            <label htmlFor="password" className="block text-gray-700 font-medium mb-2">Password</label>
            <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full p-3 border border-gray-300 text-gray-900 rounded-lg" />
          </div>
